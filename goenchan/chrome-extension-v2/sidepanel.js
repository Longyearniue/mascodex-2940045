...
document.getElementById('takeoverCsvFile')?.addEventListener('change', async (e) => {
  ...
  chrome.runtime.sendMessage({ action: 'TAKEOVER_SET_CSV', rows }, (resp) => {
    if (resp?.success) {
      ...
      // バッチ並列ボタンも有効化
      const batchBtn = document.getElementById('batchParallelStart');
...
async function refreshTakeoverState() {
  chrome.runtime.sendMessage({ action: 'GET_NEXT_COMPANY' }, (resp) => {
    ...
    const startStopBtn = document.getElementById('takeoverStartStop');
    const batchParallelBtn = document.getElementById('batchParallelStart');
    if (startStopBtn) startStopBtn.disabled = false;
    if (batchParallelBtn && total > 0) batchParallelBtn.disabled = false;
    if (resp.done) {
...
let autoPageLoadListener = null;

// 自動ループのメイン関数
async function runNextCompany(skipCurrent = false) {
  if (!autoRunning) return;

  // 現在の会社をマーク（スキップ or 初回は不要）
  if (skipCurrent) {
    await new Promise(resolve =>
      chrome.runtime.sendMessage({ action: 'MARK_DONE', status: 'skipped' }, resolve)
    );
  }

  // 次の会社を取得
  const resp = await new Promise(resolve =>
    chrome.runtime.sendMessage({ action: 'GET_NEXT_COMPANY' }, resolve)
  );

  if (!resp?.success || resp.done) {
    addTakeoverLog('🎉 全件処理済み！');
    autoRunning = false;
    document.getElementById('takeoverStartStop').textContent = '▶ 自動開始';
    document.getElementById('takeoverStartStop').classList.remove('running');
    refreshTakeoverState();
    return;
  }

  const company = resp.current;
  chrome.storage.local.set({ takeoverStarted: true });
  addTakeoverLog(`→ ${company.company_name || company.url}`);
  refreshTakeoverState();

  // プロフィール取得
  const profileData = await new Promise(resolve => chrome.storage.sync.get(null, resolve));
  const profile = {
  ...
  chrome.tabs.update(tab.id, { url: company.url });

  // ページ読込完了を待つ
  if (autoPageLoadListener) {
    chrome.tabs.onUpdated.removeListener(autoPageLoadListener);
  }

  autoPageLoadListener = function(tabId, changeInfo) {
    if (tabId !== tab.id || changeInfo.status !== 'complete') return;
    chrome.tabs.onUpdated.removeListener(autoPageLoadListener);
    autoPageLoadListener = null;

    // フォーム入力を試みる（3秒待って）
    setTimeout(() => {
      if (!autoRunning) return;

      const autoConfirm = document.getElementById('takeoverAutoConfirm')?.checked ?? false;

      chrome.tabs.sendMessage(tab.id, {
        action: 'AGENT_TAKEOVER',
        profile,
        options: { confirmBeforeSubmit: !autoConfirm }
      }, (result) => {
        if (chrome.runtime.lastError || !result) {
          // content scriptエラー → スキップ
          addTakeoverLog(`⏭ スキップ: ${company.company_name || company.url}`);
          setTimeout(() => runNextCompany(true), 500);
          return;
        }
        ...
        if (result.submitted) {
          // 自動送信完了
          addTakeoverLog(`✅ 送信: ${company.company_name || company.url}`);
          chrome.runtime.sendMessage({ action: 'MARK_DONE', status: 'done' }, () => {
            refreshTakeoverState();
            setTimeout(() => runNextCompany(false), 2000);
          });
          return;
        }
        ...
        if (result.needsConfirm) {
          // 手動確認モード → 一時停止
          addTakeoverLog(`✋ 確認待ち: ${company.company_name || company.url}`);
  ...
  chrome.tabs.onUpdated.addListener(autoPageLoadListener);
}

// 「▶ 自動開始 / ⏸ 一時停止」ボタン
document.getElementById('takeoverStartStop')?.addEventListener('click', async () => {
  if (autoRunning) {
    // 停止
    autoRunning = false;
    document.getElementById('takeoverStartStop').textContent = '▶ 自動開始';
    ...
    // 開始
    autoRunning = true;
    document.getElementById('takeoverStartStop').textContent = '⏸ 一時停止';
    document.getElementById('takeoverStartStop').classList.add('running');
    addTakeoverLog('▶ 自動開始');
    await runNextCompany(false);
  }
});
  ...
  chrome.tabs.sendMessage(tab.id, { action: 'CONFIRM_SUBMIT' }, (result) => {
    ...
    if (result?.submitted) {
      ...
      chrome.runtime.sendMessage({ action: 'MARK_DONE', status: 'done' }, () => {
        refreshTakeoverState();
        // 自動再開
        if (autoRunning) setTimeout(() => runNextCompany(false), 2000);
      });
    } else {
...
// ===== バッチ並列モード =====
async function startBatchParallelMode() {
  const batchSize = parseInt(document.getElementById('batchParallelSize').value) || 10;
  const statusEl = document.getElementById('batchParallelStatus');
  const startBtn = document.getElementById('batchParallelStart');
  startBtn.disabled = true;

  // プロフィール取得
  const profileData = await new Promise(r => chrome.storage.sync.get(null, r));
  const profile = {
    myCompany: profileData.company || '',
    myName: profileData.name || '',
    myEmail: profileData.email || '',
  ...
  const items = await new Promise(r =>
    chrome.runtime.sendMessage({ action: 'GET_NEXT_N_COMPANIES', count: batchSize }, resp => {
      r(resp?.items || []);
    })
  );

  if (items.length === 0) {
    statusEl.textContent = '❌ 処理する会社がありません';
    startBtn.disabled = false;
    return;
  }

  statusEl.textContent = `${items.length}件のタブを開いています...`;
  addTakeoverLog(`⚡ バッチ並列: ${items.length}件開始`);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    statusEl.textContent = `タブ開設中... ${i + 1}/${items.length} (✅${readyCount} ⏭${skipCount})`;

    // 新しいタブで開く（非アクティブ）
    const tab = await new Promise(r => chrome.tabs.create({ url: company.url, active: false }, r));
    ...
    if (chrome.runtime.lastError || !result) {
      ...
      try {
        chrome.scripting.executeScript({
          target: { tabId: tab.id },
          func: (name) => { document.title = '✅ 送信待ち: ' + name; },
          args: [company.company_name || company.url]
        });
      } catch(e) {}
    } else {
      skipCount++;
  ...
  statusEl.textContent = `完了: ✅${readyCount}件が送信待ち、⏭${skipCount}件スキップ。タブを切り替えて送信してください。`;
  addTakeoverLog(`⚡ バッチ完了: ✅${readyCount} ⏭${skipCount}`);
  refreshTakeoverState();
  startBtn.disabled = false;
}
...
function waitForTabLoad(tabId) {
  return new Promise(resolve => {
    chrome.tabs.get(tabId, (tab) => {
      ...
      if (tab.status === 'complete') { setTimeout(resolve, 1500); return; }
      const listener = (id, changeInfo) => {
        if (id === tabId && changeInfo.status === 'complete') {
          chrome.tabs.onUpdated.removeListener(listener);
          setTimeout(resolve, 1500); // DOM安定待ち
        }
      };
...
document.getElementById('batchParallelStart')?.addEventListener('click', startBatchParallelMode);

// 「リセット」ボタン
document.getElementById('takeoverReset')?.addEventListener('click', () => {
  chrome.storage.local.remove(['takeoverQueue', 'takeoverIndex', 'takeoverResults', 'takeoverStarted'], () => {
    ...
    document.getElementById('batchParallelStart').disabled = true;
    document.getElementById('batchParallelStatus').textContent = '';
