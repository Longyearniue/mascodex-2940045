#!/usr/bin/env node
/**
 * India Character Page Generator
 * Generates HTML character pages from profiles.
 *
 * Usage:
 *   node src/page-gen.js --pins=110001,400001,560001
 *   node src/page-gen.js                           # All profiles
 *
 * Output: data/pages/{pinCode}/index.html
 */
...
function generatePage(profile, mergedData) {
  ...
  const name = profile.name || `PIN ${pin} Mascot`;
  ...
  const district = mergedData?.district || '';

  // Build wiki "About the Area" section
  let wikiHtml = '';
  if (mergedData?.wiki?.summary) {
    wikiHtml = `<div class="section"><h2>About the Area</h2><p>${escHtml(mergedData.wiki.summary)}</p></div>`;
  }
...
<head>
  ...
  <title>${name} - PIN ${pin} Mascot | Mascodex India</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .header .pin{opacity:.7;font-size:1rem;}
...
<body>
  <script>var POSTAL='${pin}';</script>
  <div class="header">
    <a href="/in/">&larr; Back</a>
    <h1>${escHtml(name)}</h1>
    <div class="pin">PIN ${pin} &mdash; ${escHtml(city)}, ${escHtml(state)}</div>
    ${catchphrase ? `<div class="catchphrase">"${escHtml(catchphrase)}"</div>` : ''}
  </div>

  <div class="images">
    <img src="/img/in/${pin}_01.png" alt="${escHtml(name)} variant 1">
    <img src="/img/in/${pin}_02.png" alt="${escHtml(name)} variant 2">
  </div>
  ...
  <div class="content">
    <div class="section">
      ...
      <p style="margin-top:8px;opacity:.6;font-size:.85rem;">${escHtml(city)}, ${escHtml(stateName)} &bull; ${escHtml(district)} District</p>
    </div>
    ...
    <div class="chat-widget">
      ...
      <div class="chat-messages" id="chat-messages">
        <div class="chat-msg bot">Hey! I'm ${escHtml(name)} from PIN ${pin}! Ask me anything about this area!</div>
      </div>
  ...
  <div class="footer">&copy; 2025 Mascodex India. AI-generated character.</div>
...
function main() {
  ...
  console.log(`\n=== India Mascot Pipeline: Page Generation ===`);
  ...
  for (const file of profileFiles) {
    ...
    // Load merged data for wiki info
    let merged = null;
    ...
    const outDir = path.join(PAGES_DIR, pin);
    if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
    fs.writeFileSync(path.join(outDir, 'index.html'), html, 'utf8');
    console.log(`  Generated: ${pin}/index.html - ${profile.name}`);
  }
