...
**Expected Fields:**
- 会社名・部署名 (Company/Department)
- 氏名（漢字） (Name)
- 氏名（フリガナ） (Name Kana)
- メールアドレス (Email)
- メールアドレス（再入力） (Email Confirmation) - 自動入力
- 電話番号 (Phone)
- お問い合わせ内容 (Message)
...
```javascript
{
  company: 'input[name="bu_01"]',
  department: 'input[name="bu_02"]',
  name: 'input[name="name"]',
  name_kana: 'input[name="kana"]',
  email: 'input[name="mail"]',
  phone: 'input[name="tel"]',
  message: 'textarea[name="naiyo"]'
}
```

**Note:** メールアドレス確認用フィールド (`input[name="mail2"]`) は自動的に同じ値が入力されます。
...
📊 Site mapping filled 7 fields (+ 1 email confirmation)
...
6. **Verify in form:**
- [ ] 会社名・部署名（会社） field shows: テスト株式会社
- [ ] 会社名・部署名（部署） field shows: 営業部
- [ ] 氏名（漢字） field shows: 山田太郎
- [ ] 氏名（フリガナ） field shows: ヤマダタロウ
- [ ] メールアドレス field shows: test@example.com
- [ ] メールアドレス（再入力） field shows: test@example.com
- [ ] 電話番号 field shows: 090-1234-5678
...
### Hokuden Kogyo Form
- ✅ 7/7 main fields auto-filled + email confirmation field
- ✅ All fields show correct Japanese text
- ✅ Email confirmation field automatically filled with same value
...
URL: https://www.hokudenkogyo.co.jp/contact.html
Status: [PASS/FAIL]
Fields Filled: [X/8] (7 main + 1 email confirmation)
